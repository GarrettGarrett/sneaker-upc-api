export default async (req, res) => {
  if (req.method === 'POST') {
    //post stockx_id (must be parent - not size specific- because this is how scout works.) and size
    console.log(req)
    ...
    let response = await fetch(
      `https://stockx.com/api/products/${req.body.stockx_id}?includes=market,360&currency=USD&country=US`,
      requestOptions
    )
