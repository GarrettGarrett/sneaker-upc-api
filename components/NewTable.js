...
export default function NewTable({
  ...
  useLayoutEffect(() => {
    const isIndeterminate = selectedPeople.length > 0 && selectedPeople.length < result.length
                ...
                <div className="absolute top-0 left-12 flex h-12 items-center space-x-3  sm:left-16">
                    {
                                ...
                                >
                                    <button
                                        ...
                                        type="button"
                                        className={`${isEntriesDeleted(result) ? "mt-0" : "mt-6"}  inline-flex items-center rounded border border-white border-opacity-50 bg-black px-2.5 py-1.5 text-xs font-medium text-white shadow-sm hover:bg-black focus:outline-none  focus:ring-white focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-30`}
              ...
              <table className="min-w-full table-fixed divide-y divide-gray-300 ">
                <thead className="">
                  <tr>
                    <th scope="col" className=" relative w-12 px-6 sm:w-16 sm:px-8">
                      <input
                        type="checkbox"
                        className="ml-2 absolute left-4 top-1/2 -mt-2 h-4 w-4 rounded border-black text-black sm:left-6"
                        ref={checkbox}
                        checked={checked}
                        onChange={toggleAll}
                      />
                    ...
                    scope="col" className={`hover:cursor-pointer min-w-[12rem] py-3.5 pr-3 text-left text-sm font-semibold text-white`}>
                      Title
                    </th>
                    <th 
                     onClick={()=> setSizeTitle(!sizeTitle)}
                    scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-white">
                      Size
                    </th>
                    <th 
                ...
                <tr hidden={!loading}  key={123456789} className={''}>     
                  ...
                  result?.map((person, idx) => (
                    <tr hidden={!person?.title} key={idx} className={selectedPeople.includes(person) ? 'bg-black' : undefined}>
                      ...
                      <td
                        className={classNames(
                          'flex mr-10  whitespace-nowrap py-4 pr-3 text-sm font-medium',
                          selectedPeople.includes(person) ? 'text-white' : 'text-white',
                      ...
                      <td className={`${sizeTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white `}>{person.size}</td>
                      <td className={`${colorwayTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white`}>{person.color2}</td>
                      <td className={`${brandTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white`}>{person.brand}</td>
                      <td className={`${upcTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white`}>{person.upc}</td>
                      <td className={`${dateTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white`}>{person?.date ? new Date(person.date).toISOString().split('T')[0] : ""}</td>
                      <td className={`${styleIDTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white uppercase`}>{person.styleID}</td>
                      <td className={`${slugTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white`}>{person.slug}</td>
                      <td className={`${imageTitle ? " " : "opacity-0 "} whitespace-nowrap px-3 py-4 text-sm text-white`}>{person.image}</td>
                      ...
                      <td className="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        ...
                        href="#" className="text-white hover:text-white">
